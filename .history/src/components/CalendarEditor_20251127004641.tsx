...
export default function CalendarEditor({ calendar }: { calendar: Calendar }) {
  ...
  useEffect(() => {
    ...
    setDays(calendar.days.map(d => ({ ...d, nodeRef: createRef<HTMLDivElement>() })));
  }, [calendar.days]);
    ...
    <div className="flex flex-col h-screen">
      <div className="p-4 bg-white border-b flex justify-between items-center z-10 shadow-sm">
        <div>
            ...
            <div className="flex gap-2 text-sm text-gray-500 items-center">
                ...
                {calendar.isPublished ? (
                    <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded-full text-xs">Published</span>
                ) : (
        ...
        <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => router.push('/admin')}>Exit</Button>
        </div>
      </div>
            ...
            {days.map((day, index) => {
                 ...
                 return (
                    ...
                    >
                        ...
                        >
                            <span className="font-bold text-xl drop-shadow-md">{day.dayNumber}</span>
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <p className="text-sm text-gray-500">Update your calendar configuration below.</p>
            </DialogHeader>
            <form onSubmit={saveSettings} className="space-y-4">
                <div>
                    <Label htmlFor="name">Name</Label>
                    <Input id="name" name="name" defaultValue={calendar.name} required />
                </div>
                <div>
                    <Label htmlFor="slug">Slug</Label>
                    <Input id="slug" name="slug" defaultValue={calendar.slug} required />
                </div>
                <div>
                ...
                <div className="flex items-center space-x-2">
                    <Switch id="isPublished" name="isPublished" value="true" defaultChecked={calendar.isPublished} />
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <p className="text-sm text-gray-500">Configure content for this day.</p>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <Input 
                                type="number" 
                                value={day.dayNumber} 
                                onChange={(e) => setDay({...day, dayNumber: parseInt(e.target.value)})}
                            />
                        </div>
                        <div>
                            <Label>Type</Label>
                                ...
                                <div>
                                    ...
                                    {content.url && (
                                        <div className="mt-2 relative h-48 w-full bg-gray-100 rounded flex items-center justify-center overflow-hidden">
                                            <Image src={content.url} alt="Preview" fill className="object-contain" />
                                        </div>
                                    )}
                                </div>
                                <div>
