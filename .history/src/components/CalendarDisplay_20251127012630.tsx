...
export default function CalendarDisplay({ calendar }: { calendar: PublicCalendar }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white">
        <div className="max-w-md w-full p-8 bg-slate-800 rounded-lg shadow-xl">
          <h1 className="text-2xl font-bold mb-4 text-center">{calendar.name} is Locked</h1>
  ...
  return (
    <div className="min-h-screen bg-slate-900 overflow-hidden relative flex items-center justify-center">
      <div 
        className="absolute inset-0 z-0"
        style={{
            backgroundImage: calendar.backgroundUrl ? `url(${calendar.backgroundUrl})` : 'none',
      ...
      {!calendar.backgroundUrl && <div className="absolute inset-0 flex items-center justify-center text-white opacity-20 text-4xl font-bold pointer-events-none">NO BACKGROUND</div>}

      <div className="relative z-10 w-full h-full flex items-center justify-center p-4">
        <div 
            className="relative shadow-2xl"
            style={{ width: 'fit-content', height: 'fit-content', maxHeight: '100vh', maxWidth: '100vw' }}
        >
      ...
      <AnimatePresence>
        {selectedDay && (
          <DayContentModal day={selectedDay} onClose={() => setSelectedDay(null)} />
        )}
      </AnimatePresence>
    </div>
  );
...
function DayItem({ day, calendar, isTest, onOpen }: { day: Day; calendar: PublicCalendar; isTest: boolean; onOpen: () => void }) {
    ...
    const fontSize = (day as any).fontSize || (calendar as any).defaultFontSize || 40;
    ...
    return (
        ...
        >
            ...
            >
                ...
                {!canOpen && (
                    <div className="absolute inset-0 bg-gray-700 rounded-inherit opacity-100 z-0 border-2 border-gray-600 flex items-center justify-center" 
                ...
                <span className={cn("relative z-10 drop-shadow-md font-christmas", !canOpen && "text-gray-400")} style={{ fontSize: `${fontSize}cqw` }}>
                    {day.dayNumber}
                </span>
                
                {!canOpen && <Lock className="absolute bottom-[10%] z-10 text-gray-400" style={{ width: '20%', height: '20%' }} />}
            </div>
        </motion.div>
    );
}
    ...
    return (
        <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-lg bg-white/90 backdrop-blur-sm border-none shadow-2xl">
                ...
                >
                    <div className="text-4xl font-serif text-red-600 mb-2">Day {day.dayNumber}</div>
                    
                    {day.contentType === 'text' && (
                        <p className="text-lg leading-relaxed text-gray-800 font-medium">{content.text}</p>
                    )}
                        ...
                        <>
                            {content.url && (
                                <div className="relative w-full aspect-square rounded-lg overflow-hidden shadow-inner">
                                    <Image src={content.url} alt="Day content" fill className="object-cover" />
                                </div>
                            )}
                            {content.text && <p className="text-lg text-gray-800 mt-4">{content.text}</p>}
                        </>
                    )}
                    ...
                    {day.contentType === 'music' && (
                        <>
                            ...
                            {content.url && (
                                <audio controls src={content.url} autoPlay className="w-full mt-2" />
                            )}
                            {content.text && <p className="text-lg text-gray-800 mt-4">{content.text}</p>}
                        </>
                    )}
